...
.post(auth.parseToken, async (req, res) => {
    ...
    .then((offer) => {
        if (offer) {
            ...
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
            res.json({success: true, status: "Successfully created offer", offerId: offer.id});
        } else {
            res.statusCode = 400;
            res.setHeader('Content-Type', 'application/json');
            res.json({success: false, error: "Offer creation failed"});
        }
    })
    .catch((err) => {
        console.log(err);
...
router.delete('/:offerId', auth.parseToken, async (req, res) => {
    ...
    .then((rows) => {
        if (rows > 0) {
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
            res.json({success: true, status: "Successfully deleted offer"});
        } else {
            res.statusCode = 403;
            res.setHeader('Content-Type', 'application/json');
            res.json({success: false, error: "Unauthorized to delete the offer"});
        }
    })
    .catch((err) => {
        console.log(err);
